<!DOCTYPE html>
<html lang = "ru">
<head>
  <meta charset = "UTF-8">
  <meta name = "viewport" content = "width=device-width, initial-scale=1.0">
  <link rel = "stylesheet" href = "css/service.css" type = "text/css">
  <link rel = "stylesheet" href = "css/media-queries/media-queries-service.css" type = "text/css">
  <link rel = "shortcut icon" href = "assets/images/favicon.png" type = "image/png">
  <title>keimtav - студія розробки</title>
</head>
<body>
  <header>
    <div class = "header-block">
      <nav class = "menu">
        <ul class = "ul-nav">
          <li class = "li-nav"><a href="index.php">Головна</a></li>
          <li class = "li-nav"><a href = "service.php">Послуги</a></li>
          <li class = "li-nav"><a href = "order.php">Замовити</a></li>
          <li class = "li-nav"><a href = "about.php">Про нас</a></li>
        </ul>
      </nav>
      <div class = "logo">
        <img src = "assets/images/logotype.png" alt = "Логотип студії">
      </div>
      <div id = "header-text">
        <span>keimtav studio</span>
      </div>
      <div class = "topline">
      </div>
    </div>
    </header>
  <main>
    <section>
      <div class = "back-image-block">
        <div class = "header-text-about">
          <span>Послуги:</span>
        </div>
      </div>
      <div class = "about-block">
        <div class = "about-line">
        </div>
        <span id = "header-text-info">Веб-розробка:</span>
        <div class = "about-text">
          <article>
            <ul class = about-features-ul>
              <li>- лендінги</li>
              <li>- сайти візитки</li>
              <li>- інернет магазини</li>
              <li>- корпоративні сайти</li>
              <li>- форуми</li>
              <li>- соціальні мережі</li>
              <li>- зв'язок додатків і веб</li>
            </ul>
          </article>
        </div>
        <span id = "header-text-features">Десктоп-розробка:</span>
        <div class = "about-features">
          <article class = "about-features-text">
            Створення будь-яких програм, використовуючи мови: C, C++, C#, Python, Ruby, Java та ін.
          </article>
        </div>
        <span id = "header-map-location">Мобільна розробка:</span>
        <div class = "map-location">
          <article>
          <ul class = about-features-ul>
              <li>- мобільні додатки</li>
              <li>- системи веб-додаток</li>
              <li>- ігри</li>
            </ul>
          </article>
        </div>
      </div>
    </section>
  </main>
  <footer>
  </footer>
</body>
</html>
